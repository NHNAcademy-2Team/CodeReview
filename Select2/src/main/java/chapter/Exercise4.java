...
class Change {
    ...
    public void setChange(int[] answer) {
        this.quarter = answer[0];
        this.dime = answer[1];
        this.nickle = answer[2];
        this.penny = answer[3];
    }

    public String getChange() {
        String result = "";
        this.dollar = (this.quarter * 25 + this.dime * 10 + this.nickle * 5 + this.penny * 1) / 100.0;
        if (this.dollar == 0.0) {
            result = "가진 돈이 없습니다.";
        }
        result = "현재 소지금은 " + this.dollar + "달러 입니다.";
        return result;
    }
}
